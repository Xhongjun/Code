...
(function ($) {
  $.extend({
    ...
    customTable: { // 自定义表格
	      ...
	      // 初始化表格参数
	      init: function (options) {
	        $.customTable._option = options;
	        $.customTable._params = $.common.isEmpty(options.queryParams) ? $.customTable.queryParams : options.queryParams;
	        _sortOrder = $.common.isEmpty(options.sortOrder) ? "asc" : options.sortOrder;
	        _sortName = $.common.isEmpty(options.sortName) ? "" : options.sortName;
	        _striped = $.common.isEmpty(options.striped) ? false : options.striped;
	        $('#custom-table').bootstrapTable({
	          ...
	          showToggle: $.common.visible(options.showToggle),   // 是否显示详细视图和列表视图的切换按钮
	          showExport: $.common.visible(options.showExport),   // 是否支持导出文件
	          queryParams: $.customTable._params,                       // 传递参数（*）
	          columns: options.columns,                           // 显示列信息（*）
	          responseHandler: $.customTable.responseHandler            // 回调函数
	        });
	      },
	      // 查询条件
	      queryParams: function (params) {
	      ...
	      search: function (formId) {
	        var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
	        var params = $("#custom-table").bootstrapTable('getOptions');
	        params.queryParams = function (params) {
	          var search = {};
	          $.each($("#" + currentId).serializeArray(), function (i, field) {
	            search[field.name] = field.value;
	        ...
	        $("#custom-table").bootstrapTable('refresh', params);
	      },
	      // 下载-默认第一个form
	      exportExcel: function (formId) {
	        var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
	        $.modal.loading("正在导出数据，请稍后...");
	        $.post($.customTable._option.exportUrl, $("#" + currentId).serializeArray(), function (result) {
	          if (result.code == web_status.SUCCESS) {
	            window.location.href = ctx + "common/download?fileName=" + result.msg + "&delete=" + true;
	          } else {
	            $.modal.alertError(result.msg);
	      ...
	      refresh: function () {
	        $("#custom-table").bootstrapTable('refresh', {
	          url: $.customTable._option.url,
	          silent: true
	        });
	      },
	      // 查询表格指定列值
	      selectColumns: function (column) {
	        return $.map($('#custom-table').bootstrapTable('getSelections'), function (row) {
	          return row[column];
	        });
	      },
	      // 查询表格首列值
	      selectFirstColumns: function () {
	        return $.map($('#custom-table').bootstrapTable('getSelections'), function (row) {
	          return row[$.customTable._option.columns[1].field];
	        });
	      },
	      // 回显数据字典
	      selectDictLabel: function (datas, value) {
